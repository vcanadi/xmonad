...
other disp | disp == primaryDisplay = secondaryDisplay
           | otherwise = primaryDisplay

data DisplayState = DSPrimary
                  -- | DSSecondary
...
audioInc = raiseVolume 4 >> return ()
audioDec = lowerVolume 4 >> return ()
audioMute = toggleMute >> return ()

sndDisplayIsRight = True

xrandrSingle disp = safeSpawn "xrandr" [ "--output", disp,   "--auto"
                                       , "--output", other disp, "--off"]

xrandrBoth = safeSpawn "xrandr" [ "--output", primaryDisplay, "--auto"
                                , bool "--right-of" "--left-of" sndDisplayIsRight ,  secondaryDisplay
                                , "--output", secondaryDisplay, "--auto", "--rotate", "left"]
...
myKeys = \conf -> M.fromList $
    ...
    , ( (mod4Mask , xK_r)
      ...
      >> XS.get @DisplayState >>= \case
          DSPrimary   -> xrandrSingle primaryDisplay
          -- DSSecondary -> xrandrSingle secondaryDisplay
          DSBoth      -> xrandrBoth
      )
    ...
    | (key, sc) <- zip ([xK_w, xK_e]) (bool [0, 1] [1, 0] sndDisplayIsRight)
...
myConfig h = kdeConfig
    ...
    , layoutHook =  myLayout
    , handleEventHook = mconcat [ focusOnMouseMove , handleEventHook def]
    , modMask = mod4Mask
    , terminal = myTerminal
    , borderWidth = 1
